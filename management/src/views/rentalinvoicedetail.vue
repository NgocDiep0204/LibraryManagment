    ...
    <div class="max-w-7xl h-auto mx-auto bg-gray-200 mt-2 shadow-lg">
         <div class=" flex justify-between">
             <div class=" ml-1  w-full">
                 <div class="page-title">
                     <h4 class="ml-2 mb-1 text-[15px] md:text-[20px] font-bold">Rential Invoice Details List</h4>
         ...
         <div class="flex items-center mb-4">
             <div class="relative w-full max-w-md mx-auto">
                 <input  type="text" class="w-full px-10 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Search...">
         ...
         <table class="min-w-full bg-white rounded-lg sm:pl-[10px] md:pl-[40px] lg:pl-[60px]">
           <thead>
             ...
             <tbody v-if="details && details.length > 0">
               <tr v-for="(detail, index) in details" :key="detail.rentId">
                 ...
                 <td class="px-4 py-2 border-b border-gray-200 text-left">
                  {{ detail.bookIdNavigation ? detail.bookIdNavigation.name : 'N/A' }}
                 </td>  
                 <td class="px-4 py-2 border-b border-gray-200 text-left">
                     {{ detail.quantity }}
                 </td>
...
export default {
    ...
    methods: {
        async fetchDetails(rentId){
          const response = await axiosClient.get(`RentDetails/GetRentDetailById/${rentId}`);
          if (response.status === 200) {
            this.details = response.data.$values;
          console.log('Details:', response.data.$values[1])
          }
        }
